...
public class TowerSystem : BaseSystem
{
    ...
    public override void Execute()
    {
        ...
        foreach (var tower in World.Query<TowerComponent>().ToList())
        {
            ...
            foreach (var enemy in World.Query<EnemyComponent>().ToList())
            {
                if (!enemy.Has<PositionComponent>() || !enemy.Has<HealthComponent>()) continue;
            ...
            if (target != null)
            {
                ...
                if (ProjectilePrefab != null)
                    proj.Add(new UnityObjectComponent { Obj = Object.Instantiate(ProjectilePrefab, tPos.Value, Quaternion.identity) });
            }
        }
    }
}